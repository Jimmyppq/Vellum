...
El sistema SHALL implementar el store de contadores mediante una clase abstracta `RateLimitStore` con métodos `increment(key, window_seconds)` y `get_count(key)`. La implementación inicial SHALL ser `InMemoryRateLimitStore`. Una futura implementación `RedisRateLimitStore` deberá poder sustituirse sin modificar el middleware de rate limiting.
...
- **THEN** los contadores se reinician a cero (comportamiento esperado para la fase inicial; la migración a Redis resuelve la persistencia entre reinicios)
